  ...
  useEffect(() => {
    ...
    const processVoiceCommand = useCallback(async (text: string) => {
      ...
      try {
        ...
        if (!response.ok) throw new Error("Erro ao processar comando")
        
        const data: VoiceResponse = await response.json()
        setAiResponse(data)
        setInputValue(data.title || text)
        
        // Se identificou uma ação, destacar o botão correspondente
        if (data.action && data.action !== "unknown") {
          const actionElement = document.getElementById(`action-${data.action}`)
          if (actionElement) {
            actionElement.classList.add("ring-2", "ring-[#38bdf8]")
            setTimeout(() => {
              actionElement.classList.remove("ring-2", "ring-[#38bdf8]")
            }, 2000)
          }
        }
      } catch (err) {
        setError("Não foi possível processar o comando. Tente novamente.")
        console.error(err)
      } finally {
